...
def get_token():
    scopes_to_try = ["bcf", "openid offline_access bcf", "openid bcf"]
    for scope in scopes_to_try:
        try:
            ...
            if resp.status_code == 200:
                ...
                if token:
                    print(f"Token OK mit scope: {scope}")
                    return token
            print(f"Scope '{scope}' fehlgeschlagen: {resp.status_code} - {resp.text}")
        except Exception as e:
            print(f"Fehler bei scope '{scope}': {e}")
    return None

def headers():
    token = get_token()
    if not token:
        raise Exception("Kein Token erhalten - Login fehlgeschlagen")
    return {
...
@mcp.tool()
def test_login() -> str:
    """Testet ob der BIMcollab Login funktioniert und zeigt Details."""
    results = []
    scopes = ["bcf", "openid offline_access bcf", "openid bcf", "openid"]
    for scope in scopes:
        try:
            resp = httpx.post(TOKEN_URL, data={
                "grant_type": "client_credentials",
            ...
            results.append(f"Scope '{scope}': Status {resp.status_code} -> {resp.text[:200]}")
        except Exception as e:
            results.append(f"Scope '{scope}': FEHLER -> {e}")
    return "\n\n".join(results)
